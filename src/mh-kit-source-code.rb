...
sleep(2)
puts "I am a virtual mental health toolkit that will help you get back on your feet when times get tough.\n
...
#DEVNOTE: loop for repeating the menu after an option has been chosen.Needed for navigation purposes and error handling.
if user_input != 5
  loop do
    ...
    def mood_journal #collects 'mood stats' which will be used in the other menu option 'mood chart' once feature is added. Will not be implemented prior to assignment submission due to time restraints. However the logic required to do so is well understood.
      puts
    ...
    def mood_chart #---IMPORTANT DEVNOTE BELOW!!!:---
      sleep(1) #i could assign the (mood_journal) variable to this class and therefore display real statistics however being a local variable the data would be wiped after a session is ended. due \n
      #...to time restraints i have chosen to use manually inputted dummy data so that i am able to display the capabilites of my application in the presentation. \n
      #...If im correct i could store variable data without a database via a session logging gem thus saving the data to another file or by using rubies built in dateTime function, but due to presentation purposes i have chosen to do this.
      puts
